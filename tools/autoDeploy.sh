...
rm -rf accounts.txt
# 结束之前的所有启动进程
ps -aux | grep $EngineFile | awk '{print $2 }' | xargs sudo kill -9
while (($int <= $2)); do
    filename="bee_"$int""
    ...
    # 启动
    rm -rf output.log
    ../cmd/$6/$EngineFile start --config ./$BeeYamlVersion > output.log &
    # 输出地址
    if [ -d .bee ]; then
        echo "$int账户已创建"
    else
    ...
    ../exportKeys/exportKeys_$6.exe .bee/keys/ $3 | awk 'NR==3{print $3}' >>../accounts.txt
    echo "============================================================" >>../accounts.txt
    echo "$int节点账户导出成功!"
    cd ..
    let "int++"
